...
# DEV environment — mirrors prod pattern with a dev-specific Pages project.
#
...
resource "cloudflare_dns_record" "pages_cname" {
  ...
  comment = "Cloudflare Pages dev apex — managed by terraform"
...
output "subdomain" {
  ...
  description = "Default *.pages.dev hostname for the dev project."
}

output "custom_domain" {
  value       = cloudflare_pages_domain.custom.name
  description = "Dev custom domain attached to the Pages project."
}
